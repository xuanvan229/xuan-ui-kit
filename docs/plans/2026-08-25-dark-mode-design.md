...
## Why
The kit is the single source of truth for the brand look; apps built on it need a dark theme that is *designed* (now done in `docs/dark/`), not derived, and that consumers get for free from the same registry.
...
4. **Preview toggle:** `app/preview/theme.tsx` (`PreviewTheme`) — a button that toggles `document.documentElement.classList` and persists to `localStorage`; a tiny inline script in `app/layout.tsx` applies the stored class before paint (no `useEffect`). `<meta name="color-scheme">`/`color-scheme` on `html` follows the class.
